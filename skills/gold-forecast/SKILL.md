---
name: gold-forecast
description: "黄金交易预测日报。确定性脚本从 LBMA/FRED/CFTC/东财采集数据并算出量化基线,MiniMax-M3 在基线上做调整并撰写七段报告,C1-C16 自检通过后入库、归档、推飞书;到期预测自动结算并记入三方对照(最终/基线/朴素)记分卡。触发词:黄金预测、金价预测、gold forecast、黄金日报、伦敦金、定盘价、黄金基线、黄金回测、黄金结算、金价区间。"
---
...
`validate.js` 的 C1–C16**。此处只列要点,细则不在本文档复述以免两处走样:

- 输出 = 开头一个围栏 JSON 块(承载全部可判定字段)+ 其后七段中文正文
- **七段标题写死在 `forecast-parser.js` 的 `SECTION_TITLES`**(今日结论 / 市场事实 /
  对手盘结构 / 判断依据 / 上期结算与反思 / 如何使用本区间 / 免责声明),契约与 C15 从这一处取值。
  契约要求逐字、C15 只查前缀,契约更严的方向是安全的
- 正文数字小数位 ≤4(C16)。4 位的舍入误差 ≤5e-5,远在 C4 的 0.5% 与 C14 的 0.001 之内,不会打架
- **C14 与 C4 分层**:C14 只管「本报告给出的预测概率与 JSON 一致」,判据是
  **概率锚点(`概率` 或 `prob_up`)与数字落在同一子句**;归属他方的概率
  (基线 / 朴素 / 上期 / 前值)与「自 X 至 Y」的起点**豁免 C14** —— 契约要求第一段写
  「相对基线的调整幅度」,不豁免就是自检在阻止报告满足契约。豁免的只是一致性判定,
  **出处仍归 C4**,编造的基线概率照样被拦
- **C4 的两类豁免**:markdown 有序列表序号;以及 `<中文标签> (<字段代码>)` 里标签部分的数字
  (`标普 500 指数 (eastmoney.SPX)`、`华安黄金 ETF 518880 (eastmoney.518880)`)。
  字段代码必须**在 prompt 真实存在的键名集合里**才给豁免,编出来的 `(target)` 拿不到
