...
const { freshTmp, FIXTURE, withBlockNetwork } = require('./helpers');
...
function runScript(script, args, { home, env = {} }) {
  const r = spawnSync('node', [SCRIPT(script), ...args], {
    ...
    // NODE_OPTIONS 排在 ...env 之后,调用方顶不掉。
    env: { PATH: process.env.PATH, HOME: home, ...env, NODE_OPTIONS: withBlockNetwork(env.NODE_OPTIONS) },
  });
