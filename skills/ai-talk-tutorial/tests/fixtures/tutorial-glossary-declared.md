...
| 教程写法 | 转录原文 | 依据 |
|---|---|---|
| DeepChem | Deep Chem | 转录为 ASR 分词写法;指开源 FP8 GEMM kernel 库 |
| M.2 | M dot | 转录只出现 M dot one(即 M.1),M.2 为其下一代版本号,依上下文推断 |
| DDP | distributed data parallel | 转录用全称,教程改用业界通用缩写 |
| MultiPL-E | multiple E | ASR 把 benchmark 名 MultiPL-E 听成 multiple E |
| BigCodeBench | big code bench | 转录为 ASR 分词写法,教程用官方拼写 |
| Qwen3-360 | turn 360 | ASR 误听;转录未给出准确型号,依上下文推断为 Qwen3 系列 |
| DeepSeek | deep seek | 转录为 ASR 分词写法,教程用官方拼写 |
| EvalPlus | E valve plus | ASR 把 benchmark 名 EvalPlus 听成 E valve plus |
