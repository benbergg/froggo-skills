...
4. 方法论**至少 3 步**,每步须有标题和正文。**各段有最小内容量下限**(C1/C5 会逐条硬校验,
   不达标 exit 5 中止,不是建议值):H1 标题 ≥6 字符、TL;DR ≥2 条、背景段 ≥30 字符、
   checklist ≥2 条、**每个步骤正文 ≥15 字符**。这些是防"结构合法但内容空洞"的下限,
   不是目标值 —— 正常产出应远高于它们(实测参考:背景段 431 字符、每步正文 260+ 字符)。
