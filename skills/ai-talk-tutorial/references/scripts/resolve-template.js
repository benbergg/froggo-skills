...
// and cross-checks fields against the expected field used by ai-talk-tutorial.
// 字段名以钉钉后台实际配置为准,不符只 WARN 不阻断。
//
// Two modes:
...
//
// 同源:整体复制自 skills/exp-compass-daily/references/scripts/resolve-template.js,
// 仅改 EXPECTED_FIELDS、缓存目录名两处。这是**刻意决策**(2026-07-27 裁决):
// froggo-skills 的每个 skill 需能独立 rsync 到 VM 运行,不引入跨 skill 的共享模块依赖。
// 钉钉 get-template 契约变化时,两处都要改。
...
function usage() {
  return [
    ...
    '  --cache <path>            Cache file path (default: $HOME/.cache/ai-talk-tutorial/template.json)',
...
function defaultCachePath() {
  return path.join(os.homedir(), '.cache', 'ai-talk-tutorial', 'template.json');
}

function inferBin() {
  const root = process.env.CLAUDE_PLUGIN_ROOT;
  if (root) return path.join(root, 'skills', 'dingtalk-log', 'scripts', 'dingtalk-log.js');
  // Fallback: walk up from this file's location: …/ai-talk-tutorial/references/scripts/
