...
## Bug 加备注：v1 / v2 RESTful API 均不支持

实测 + 官方文档确认（[v1 索引](https://www.zentao.net/book/api/665.html) §2.14、[修改 Bug 723](https://www.zentao.net/book/api/723.html)）：

- v1 bug 章节 9 个端点（`720`/`721`/`722`/`723`/`724`/`1120`/`1121`/`1142`/`1181`）**没有「添加备注/评论」端点**
- v2 bug 章节 10 个端点（`2192`-`2244`）也没有
- 「修改 Bug」（`PUT /bugs/{id}`）请求体 15 个字段中**没有 `comment` 字段**
- 实测盲试 8 种端点（`POST /bugs/{id}/comments` `POST /bugs/{id}/comment` `POST /bugs/{id}/actions` `POST /actions/bug/{id}/createComment` `POST /actions` `POST /comments/bug/{id}` `POST /bugs/comments` `PUT /bugs/{id}` body `{"comment":...}`）：要么 404，要么 200 但**无副作用**（actions 表无 `commented` 类型记录写入）

**结论**：禅道 RESTful API 当前版本不暴露此能力。如必须加备注，只能通过 UI 或非 v1 老路由（`/index.php?m=action&f=createComment`，需要 cookie session，超出本 skill 范围）。
