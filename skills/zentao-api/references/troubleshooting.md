...
## 实测白名单（详见 endpoints.md ## Query Parameter Whitelist）

具体参数白名单已迁移到 `endpoints.md ## Query Parameter Whitelist`，本文件不再重复。

## Server-Side Filter Unreliable

API v1 的服务端过滤参数除了 `/executions?status=` 之外几乎都不可信。设计上**强制改用客户端 jq 筛**，避免依赖服务端。

## Control-Char-in-Response Already Handled
...
## Bug Comments via Action Endpoints
...
## Empty Body on Old Closed Projects
...
## §11.2 List Endpoints Hide Subtasks in .children[]
...
## §11.3 /products/{pid}/bugs Default Filters status != closed
...
## §11.1 bytenew.com Instance Behaviors
