...
凭据(`ZENTAO_*`)由 shell 全局环境变量提供(配置在 `~/.zshrc` / `~/.profile`),脚本只读 `process.env`,不再 source 任何 `.env` 文件。skill 自带脚本用 `${CLAUDE_PLUGIN_ROOT}` 定位——Claude Code 加载 skill 时自动注入,无需用户配置且自动跟版本;openclaw 等环境由其编排层用安装目录绝对路径(如 `~/.openclaw/skills/...`)调用,不依赖该变量。知识库根目录同样不经环境变量传入——按所在环境的全局规则解析(Claude Code 读 `CLAUDE.md`、openclaw 读 `AGENTS.md`)。

## 主流程(Step 0-5)

### Step 0 Setup

凭据应已在 shell 全局环境变量中(配置在 `~/.zshrc` / `~/.profile`)。校验已注入:
...
**采集脚本正常耗时约 150-160 秒**(2026-07-27 实测 156s,走查 112 个 execution;禅道尾延迟抖动时可能翻数倍)。必须按下面的方式跑,否则会以 cron 整轮超时收场。
