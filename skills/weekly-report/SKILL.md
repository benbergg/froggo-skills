---
name: weekly-report
description: "产研周报自动生成。Node 脚本从禅道采集本周完成任务/解决 Bug/待跟进 Bug/下周计划为单一 JSON,AI 在对话内按模板撰写周报,跑 7 项断言自检,人工预览确认后写入知识库 05-Reports/weekly/、git push、发送飞书摘要。触发词:周报、研发周报、产研周报、weekly report、青蛙周报、本周完成、本周 Bug、下周计划。"
---
...
`user-invocable: true`,可手动 dry-run;也可由定时任务(如 cron `0 18 * * 6`)自动触发。

## 必填环境变量

| 变量 | 必填 | 说明 |
|---|---|---|
| `WEEKLY_REPORT_DIR` | ✓ | weekly-report skill 根目录,用于定位本 skill 脚本(替代 Claude Code 专有的 `CLAUDE_PLUGIN_ROOT`,兼容 openclaw / codex);各环境在 shell 全局环境变量配置 |
...
| `WEEKLY_PRODUCT_IDS` | – | 产品 ID 白名单 CSV(如 `95,114`),缺省 `[95]` (VOC,见 `DEFAULT_PRODUCT_IDS`);临时借派到其他 product 时设置 |
| `WEEKLY_API_BUDGET` | – | API 调用上限,缺省 `1500` (V4 实测 ~275 已够);加 product 后再 budget 触顶时按 stderr 提示加到 `4000+` |
| `WEEKLY_HARD_TIMEOUT_MS` | – | wall-clock 上限毫秒,缺省 `600000` (10 min) |
| `AI_DAILY_DIR` | – | 缺省 知识库的 `08-Research/AI-Daily/Daily`,AI Daily 源目录 |

凭据与脚本路径(`ZENTAO_*`、`WEEKLY_REPORT_DIR`)由 shell 全局环境变量提供(配置在 `~/.zshrc` / `~/.profile`),脚本只读 `process.env`,不再 source 任何 `.env` 文件。知识库根目录不经环境变量传入——按所在环境的全局规则解析(Claude Code 读 `CLAUDE.md`、openclaw 读 `AGENTS.md`)。

## 主流程(Step 0-5)

### Step 0 Setup

凭据与 `WEEKLY_REPORT_DIR` 应已在 shell 全局环境变量中(配置在 `~/.zshrc` / `~/.profile`)。校验已注入:

```bash
for v in WEEKLY_REPORT_DIR ZENTAO_BASE_URL ZENTAO_ACCOUNT ZENTAO_PASSWORD; do
  [ -n "${!v}" ] || echo "MISSING env: $v (请加到 ~/.zshrc 或 ~/.profile)"
done
```

缺则在 Step 1 前停下并提示补哪些。计算本周 ISO 周由 collect-weekly.js 内置完成,无需 shell 操作。

### Step 1 数据采集

```bash
# 采集本周(默认 product=[95])
node ${WEEKLY_REPORT_DIR}/references/scripts/collect-weekly.js \
...
```bash
# KL = 知识库根目录,按所在环境的全局规则解析(Claude Code 读 `CLAUDE.md`、openclaw 读 `AGENTS.md`)后填入具体路径
KL="<知识库根目录>"
DEST="$KL/05-Reports/weekly/${WK_NUM}-工作周报.md"
cp /tmp/weekly-${WK_NUM}.md "$DEST"
cd "$KL" && git add 05-Reports/weekly/${WK_NUM}-工作周报.md \
...
```
<知识库根目录>/05-Reports/weekly/${WK_NUM}-工作周报.md
```
...
| 应用场景 | AI工具 | 效果 |
|----------|--------|------|
| 禅道数据整理 | AI Agent + zentao-api | 通过 API 自动提取任务和 Bug 数据 |
| 周报生成 | AI Agent | 自动汇总、结构化输出,含 7 项断言自检闸门 |
| 知识沉淀 | AI Agent + 知识库 | 自动保存、提交并推送 |
| (其他本周 AI 应用,至少补 1 行) |  |  |
...
- **写入路径固定** 知识库的 `05-Reports/weekly/${WK_NUM}-工作周报.md`,用 `cp` 工具(不用 obsidian-cli);知识库根目录按所在环境的全局规则解析(Claude Code 读 `CLAUDE.md`、openclaw 读 `AGENTS.md`),不经 skill 环境变量传入
