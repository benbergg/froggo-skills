...
> 本 skill 不直接调用 lib(V3 已废 lib),所有禅道访问走 `zentao-api/references/auth-and-curl.md` 的 6 个 bash snippet。
...
#    把 references/auth-and-curl.md 中 ```bash 块全部复制到当前 shell
...
| PUT 任务/Bug action 静默 no-op | V3 已知问题,改用 POST(详见 zentao-api/known-issues.md §11) |
| `.parent` 字段为 -1 | 该任务自身是父任务,**不要**回查父(sentinel,详见 zentao-api/known-issues.md §11.1) |
...
- **不直接调用 zentao-api 的 lib**:V3 已废,必须走 auth-and-curl.md 6 snippet。
