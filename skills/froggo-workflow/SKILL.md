...
> [!danger] 开发阶段边界约束
> - **不得新增需求** - 开发阶段只执行已确认的设计
> - **不得修改设计意图** - 设计文档是权威，开发只是执行
> - **发现问题必须回退** - 如发现设计不足或新需求，立即中断并返回阶段2（设计）
...
**执行内容：**
1. 调用 `git-commit` skill 提交代码
2. 遵循 Conventional Commits 规范
3. 格式：`<type>: <description> #<zentao_id>`
...
| commit 信息不规范 | 遵循 `git-commit` skill 的 Conventional Commits 格式 |
| 开发阶段自行新增需求 | 发现新需求必须回退到设计阶段 |
| 开发阶段修改设计意图 | 设计是权威，开发只执行 |
...
4. **Claude Code 是流程协调者** - 不是自由发挥的 coder
   - 不确定 → 调研
   - 需求变化 → 回退阶段
   - 超出设计 → 中断并确认用户
