...
### 4.4 主动人化：进站前先推演"真人会怎么逛"

对照表（4.2）是**被动闸门**（动作前拦红旗），但不够——模型天然倾向走最短路径直奔目标，而"一步到位的最优路径"本身就是反人类指纹。所以模型在动手前要先**主动推演一个真人的浏览会话**：

- **设人设与意图**：我现在是个想买 X 的真人，预算多少、为什么逛、在意什么 → 行为有动机锚点
- **规划真人节奏的会话弧线**：先搜 → 扫几个结果 → 点开 1–2 个看看 → 可能返回再比 → 可能被相关推荐带偏 → 才决定。**不是**搜完直接跳目标商品
- **保留犹豫与非线性**：真人会回退、改搜索词、停下读评价；机器的"最优直达"是指纹
- **显式写出来**：在 thinking 中先说清"这个真人接下来会怎么动"，再落到 vortex 工具调用

> 核心问法转换：不要问"怎么完成任务"，要问"一个真人为了这个目的会怎么逛"——然后照那个人的样子操作。

---

## 5. 强制前置自检回路（skill 脊柱 — 每次 vortex 访问目标站点前必走）

skill 的操作骨架不是静态清单，而是把"像不像人"的判断**嵌进每一次访问的执行路径**：

```
1. 想清这次要做什么访问（导航 / 点击 / 翻页 / 提取…）
2. 自检：这样访问像不像一个真人在逛？
   ├─ 对照「反人类行为对照表」(4.2) 看有没有踩红旗
   └─ 对照「真人会怎么逛」推演 (4.4) 看是不是直奔目标的最短路径
3. 不像人 → 先修正方案：
   换搜索页入口 / 换 mouse_drag 真实点击 / 加随机停顿 / 降频 / 调整会话弧线
4. 像人了 → 才真正调用 vortex 工具执行
5. 执行后 debug_read 扫风控信号 → 命中走 §7 退避决策树
```

三层闭环：**主动推演 (4.4) 生成像人的方案 → 被动闸门 (4.2) 执行前拦红旗 → 前置自检回路把两者强制串进每次访问，不像就修正再动手。**
...
定位：这是回路的一个应用示例，不是唯一正确流程。模型应以"像不像真人"为准绳灵活变通。
